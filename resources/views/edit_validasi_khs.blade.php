...
@section('main')
    ...
    <div class="judul_page">
        <h3>INFORMASI PERWALIAN / PENYETUJUAN KHS /EDIT KHS</h3>
    </div>
    <form action="/confirmEditKHS" method="post" enctype="multipart/form-data">
        @csrf
        <div class="form-row">
        ...
        <hr class="pemisah-input">
        <div class="form-row">
            <label id="label-ip" for="ip">Ip Semester : </label>
            <input type="number" name="ip" class="ip" id="ip" maxlength="2" value="{{ $dataEdit->ip_smt }}" required step="0.01">
        </div>
        <hr class="pemisah-input">
        ...
        <div class="form-row" id="dokumen-upload">
            ...
            <div class="container-icon">
                <a  href="{{ asset('dokumen_ppl_projek/khs/'. $dataEdit->scan_khs)}}" id="link_document" target="_blank"><img id="gambar-download" src="{{ asset('gambar/icon-download.png') }}" width="30px" height="30px" alt="download"></a>
            </div>
        </div>
        <hr class="pemisah-input">
        <div class="container-btn">
            <div class="wadah-btn"><input type="submit" id="btn-submit" class="btn" value="Edit KHS"></div>
            <a id="delete" href={{ route('delete_khs', ['nim' => urlencode($dataEdit->nim),'smt' => urlencode($dataEdit->smt)]) }} class="btn-action2" >Delete</a>
        </div>
    </form>
@endsection