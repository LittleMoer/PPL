...
<title>PENYETUJUAN | PKL</title>
@section('main')

<div class="judul_page">
    <h3>INFORMASI PERWALIAN / PENYETUJUAN PKL</h3>
</div>
@if(session('success'))
<div class="flash">
  {{ session('success') }}
...
<div class="menu_nav">
  <div class="container-tabel-menu-atas">
    <table class="table-menu-atas" cellspacing="10">
      <thead>
        <tr class="tr-1" >
          <th scope=""><a class="link-menu" href="/unverif_pkl">Unverified</a></th>
          <th scope=""><a class="link-menu" href="/verif_pkl">verified</a></th>
        </tr>
      </thead>
    </table>
  </div>
          ...
          <tbody>
            @foreach ($pkl as $item)
                <tr class="data">
                    <td >{{ $item->nama }}</td>
                    <td >{{ $item->nim }}</td>
                    <td >{{ $item->nilai_pkl }}</td>
                    <td >{{ $item->semester }}</td>
                    <td ><a href="dokumen_ppl_projek/pkl/{{ $item->scan_pkl }}" target="_blank" id="download">Lihat File</a></td>
                    <td>
                      @if ($item->stat_cek == 1)
                        <h4 id="sudah_disetujui" style="color: green;">Verified</h4>
                      @else
                        <a id="setuju1" href={{ route('edit_validasi_pkl', ['nim' => urlencode($item->nim)]) }} class="btn-action">Edit</a>
                        <a id="setuju" href={{ route('setujui_pkl', ['nim' => urlencode($item->nim)]) }} class="btn-action1">Approve</a>
                      @endif
                    </td>
                </tr>
            @endforeach
